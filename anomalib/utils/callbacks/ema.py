...
class EMACallback(pl.Callback):
    ...
    NOTE: update steps mean the number of optimization steps
    """

    def __init__(self, decay: float = 0.9999, update_steps: int = 10, steps_start: int = 2000,
                 ema_device: Optional[Union[torch.device, str]] = None, pin_memory=True):
        super().__init__()
        self.decay = decay
    ...
    def start(self, trainer: "pl.Trainer", pl_module: pl.LightningModule) -> None:
        ...
        if not self._ema_state_dict_ready and pl_module.global_rank == 0:
            ...
            if self.ema_device is None:
                self.ema_device = pl_module.device

            if self.ema_device:
                self.ema_state_dict = {k: tensor.to(device=self.ema_device)
    ...
    def on_train_batch_end(self, trainer: "pl.Trainer", pl_module: pl.LightningModule, *args, **kwargs) -> None:
        ...
        if global_step < self.steps_start:
            return
        if global_step == self.steps_start:
