...
# NOTE: correct the url for MVTec LOCO dataset
...
def make_mvtec_loco_dataset(
    ...
    root = Path(root)
    samples_list = [(str(root),) + f.parts[-3:] for f in root.glob(r"*/*/*") if (f.suffix in extensions or f.is_dir()) and not f.name.startswith(".")]
    if not samples_list:
...
class MVTecLOCODataset(AnomalibDataset):
    ...
    def _setup(self) -> None:
        self.samples = make_mvtec_loco_dataset(self.root_category, split=self.split, extensions=IMG_EXTENSIONS)
    ...
    def __getitem__(self, index: int) -> dict[str, str | Tensor]:
        ...
        if self.task == TaskType.CLASSIFICATION:
            ...
            if label_index == 0:
                ...
                assert mask_path.exists() and mask_path.is_dir(), f"Mask path {mask_path} does not exist."
                for mask_file in mask_path.glob("*"):
                    if mask_file.suffix not in IMG_EXTENSIONS:
                        continue
                    region_mask = cv2.imread(str(mask_file), flags=0)
