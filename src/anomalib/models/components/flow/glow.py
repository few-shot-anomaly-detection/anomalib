...
def fetch_dataloader(args, train=True, data_dependent_init=False):
    ...
    dataset = {'mnist': MNIST, 'celeba': None}[args.dataset]
...
class Invertible1x1Conv(nn.Module):
    ...
    def __init__(self, n_channels=3, lu_factorize=False):
        ...
        w = torch.randn(n_channels, n_channels).cuda()
        w = torch.linalg.qr(w)[0]  # note: nn.init.orthogonal_ returns orth matrices with dets +/- 1 which complicates the inverse call below  # type: ignore
