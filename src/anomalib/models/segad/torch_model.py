...
class Encoder(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        # self.enconv6 = nn.Conv2d(64, 64, kernel_size=7, stride=1, padding=0)
        self.enconv5 = nn.Conv2d(64, 64, kernel_size=4, stride=2, padding=1)
        self.enconv6 = nn.Conv2d(64, 32, kernel_size=4, stride=2, padding=1)
        self.enconv7 = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=0)
...
class Decoder(nn.Module):
    ...
    def __init__(self, out_channels, padding, img_size, *args, **kwargs) -> None:
        ...
        self.last_upsample = img_size
        self.deconv1 = nn.Conv2d(16, 32, kernel_size=4, stride=1, padding=2)
        self.deconv2 = nn.Conv2d(32, 64, kernel_size=4, stride=1, padding=2)
        self.deconv3 = nn.Conv2d(64, 64, kernel_size=4, stride=1, padding=2)
        self.deconv4 = nn.Conv2d(64, 64, kernel_size=4, stride=1, padding=2)
        self.deconv5 = nn.Conv2d(64, 32, kernel_size=4, stride=1, padding=2)
        self.deconv6 = nn.Conv2d(32, 16, kernel_size=4, stride=1, padding=2)
        self.deconv7 = nn.Conv2d(16, 8, kernel_size=3, stride=1, padding=1)
        self.deconv8 = nn.Conv2d(8, out_channels, kernel_size=3, stride=1, padding=1)
...
class SegADModel(nn.Module):
    ...
    def __init__(
        ...
        self.ae: AutoEncoder = AutoEncoder(out_channels=self.out_channels, padding=padding, img_size=input_size[0])
        self.student = PDN_M(out_channels=out_channels, padding=padding)
        self.input_size: tuple[int, int] = input_size
        self.loss = nn.CrossEntropyLoss(reduction='none')

    def forward(self, batch: Tensor, seg_batch: Tensor = None) -> Tensor | dict:
        ...
        # output = self.ae(seg_batch)
        # distance = torch.pow(
        #     output - seg_batch, 2
        # ).mean(dim=1)
        distance_stae = torch.pow(
            student_output - ae_output, 2
        ).mean(dim=1)
        distance_ae = self.loss(
            ae_output, seg_batch
        )
        if self.training:
            loss_ae = torch.mean(distance_ae)
            loss_stae = torch.mean(distance_stae)
            return loss_ae, loss_stae
        else:
            return distance_stae
